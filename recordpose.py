...
output_directory = os.path.join(
    r"D:\exp_pose2sim\pose2sim\Pose2Sim\Empty_project\raw-2d",
    current_datetime
)
os.makedirs(output_directory, exist_ok=True)

# 影片檔案名稱
output_file_cam1 = os.path.join(output_directory, 'vid_cam1.mp4')
output_file_cam2 = os.path.join(output_directory, 'vid_cam2.mp4')

# 初始化兩個VideoCapture物件，代表兩台webcam
cam1 = cv2.VideoCapture(0)  # 修改為第一台webcam的索引
...
out_cam1 = cv2.VideoWriter(output_file_cam1, fourcc, fps, (width, height))
...
def record():
    ...
    while True:
        if recording:
            ret_cam1, frame_cam1 = cam1.read()
            ret_cam2, frame_cam2 = cam2.read()

            if ret_cam1 and ret_cam2:
